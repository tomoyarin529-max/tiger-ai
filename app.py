...
st.write("⚙️ 一括丸投げ仕分け ＆ 全レースワイド3点（300円）リアルタイムオッズ完全連動モデル")
    ...
    if uploaded_files:
        for f in uploaded_files:
            try:
                ...
                # 🔑 【超確実】列名ではなく、ファイル名そのもので100%完璧に仕分ける羅針盤回路！
...
if df_horse_raw is not None and df_race_raw is not None:
    ...
    # 🧮 【超重要・型変換パッチ】オッズCSVの比較用数値を強制的に完全クリーンアップ！
    ...
    # ==========================================
    st.markdown("### 📡 機能①：全レース横断・最強ワイド3点 ＆ 【オッズ完全連動】スパイラルボード")
    
    all_wide_matches = []
    tracks = df_horse_raw["競馬場"].unique()
    
    # 🧠 オッズCSVからワイドオッズをピンポイント抽出する秘密関数（スコープエラー完全対応版）
    def get_wide_odds_str(odds_df, track, race, horse_a, horse_b):
        if odds_df is None or "賭式" not in odds_df.columns:
            return ""
        try:
            b1 = min(int(horse_a), int(horse_b))
            b2 = max(int(horse_a), int(horse_b))
            df_w = odds_df[
                (odds_df["競馬場"] == track) & 
                (odds_df["レース番号"] == int(race)) & 
                (odds_df["賭式"] == "ワイド") & 
                (odds_df["番号1"] == b1) & 
                (odds_df["番号2"] == b2)
            ]
            if not df_w.empty:
                o_min = df_w.iloc[0]["オッズ"]
                o_max = df_w.iloc[0]["オッズ（最大）"]
                if pd.isna(o_max) or o_min == o_max:
                    return f" [{o_min}倍]"
                return f" [{o_min}～{o_max}倍]"
        except:
            pass
        return ""
    ...
    for track in tracks:
        ...
        for r in r_list:
            ...
            if len(df_r) >= 3:
                ...
                # 🎯 ワイドの各買い目のろにオッズを自動結合！
                o12 = get_wide_odds_str(df_odds_raw, track, r, n1, n2)
                o13 = get_wide_odds_str(df_odds_raw, track, r, n1, n3)
                o23 = get_wide_odds_str(df_odds_raw, track, r, n2, n3)
                
                wide_combos = f"① {n1}-{n2}{o12}\n② {n1}-{n3}{o13}\n③ {n2}-{n3}{o23}"
                ...
                all_wide_matches.append({
                    ...
                    "大本命馬": f"{n1}番 ({top3.loc[0, '馬名']}){win_odds_str}",
                    "AI推奨度": f"{win_rate} ％",
