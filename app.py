...
if df_horse_raw is not None and df_race_raw is not None:
    ...
    # 🧠 オッズCSVからワイドオッズをピンポイント抽出する秘密関数
    def get_wide_odds_str(track, race, horse_a, horse_b):
        if df_odds_raw is None or "賭式" not in df_odds_raw.columns:
            return ""
        try:
            b1 = min(int(horse_a), int(horse_b))
            b2 = max(int(horse_a), int(horse_b))
            df_w = df_odds_raw[
                (df_odds_raw["競馬場"] == track) & 
                (df_odds_raw["レース番号"] == int(race)) & 
                (df_odds_raw["賭式"] == "ワイド") & 
                (df_odds_raw["番号1"] == b1) & 
                (df_odds_raw["番号2"] == b2)
            ]
    ...
    for track in tracks:
        ...
        for r in r_list:
            ...
            if len(df_r) >= 3:
                ...
                # 🎯 【超進化】ワイドの各買い目の後ろにオッズを自動ドッキング！
                o12 = get_wide_odds_str(track, r, n1, n2)
                o13 = get_wide_odds_str(track, r, n1, n3)
                o23 = get_wide_odds_str(track, r, n2, n3)
