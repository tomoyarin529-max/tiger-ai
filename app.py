...
# 🚀 【作戦1】まず、Streamlit Cloudの「隠し金庫（Secrets）」に鍵があるか確認
if "TOKEN" in st.secrets:
    TOKEN = st.secrets["TOKEN"]
# 💻 【作戦2】クラウドになければ、PCローカルの「line_key.txt」を探しに行く
else:
...
uploaded_files = st.file_uploader("📋 CSVファイルをまとめてここにドロップ！", type=["csv"], accept_multiple_files=True, key="local_csv_uploader")
if uploaded_files:
