...
# 3. 起動時の致命的クラッシュを100%防衛するグローバル・セーフティネット
try:
    ...
    def calc_true_ai_score(row):
        ...
        return max(35, min(99, int(76 + (place_rate * 20) + blood_bonus - weight_penalty + odds_effect + rand_val)))
    ...
    def get_wide_odds_float(odds_df, track, race, horse_a, horse_b, date=None):
        try:
            ...
            c_tr = (odds_df["競馬場"] == track)
            c_rc = (odds_df["レース番号"] == int(float(race)))
            c_ty = (odds_df["賭式"] == "ワイド")
            c_n1 = (odds_df["番号1"] == b1)
            c_n2 = (odds_df["番号2"] == b2)
            cond = c_tr & c_rc & c_ty & c_n1 & c_n2
    ...
    if mode in ["地方競馬（実戦・当日ZIP丸投げ）", "中央競馬（JRAコピペ）"]:
        ...
        with col2:
            total_budget_per_race = st.number_input("💵 1レースあたりの総投資予算（円）", min_value=300, value=1000, step=100)
        ...
        if df_horse_raw is not None and df_race_raw is not None:
            ...
            if df_odds_raw is not None:
                ...
                if "賭式" in df_odds_raw.columns:
                    df_wo_sub = df_odds_raw[df_odds_raw["賭式"] == "単勝"]
                    df_wo_cols = ["競馬場", "レース番号", "番号1", "オッズ"]
                    df_win_odds = df_wo_sub[df_wo_cols].rename(columns={"番号1": "馬番", "オッズ": "リアルタイム単勝オッズ"})
                    ...
                    if len(df_r) >= 3:
                        ...
                        if df_race_raw is not None and not df_race_raw.empty:
                            ...
                            if not df_time_search.empty:
                                if "発走時刻" in df_time_search.columns:
                                    race_time_str = str(df_time_search.iloc[0]["発走時刻"])
                                else:
                                    race_time_str = str(df_time_search.iloc[0]["発走予定時刻"])
                        ...
                        if ana_horse_row is not None:
                            ana_signal = f"🔥 LOCKON!! 【 {int(float(ana_horse_row['馬番']))}番 】 ({ana_horse_row['馬名']}) [{ana_horse_row['リアルタイム単勝オッズ']}倍]"
                        else:
            ...
            if df_master_horse is not None and df_master_payback is not None and df_master_odds is not None:
                ...
                if st.button("⚔️ 過去データ検証作戦（バックテスト）を開始せよ！"):
                    ...
                    with st.spinner("🧠 AIが過去の全レースを脳内シミュレーション中..."):
                        df_wo_sub = df_master_odds[df_master_odds["賭式"] == "単勝"]
                        df_wo_cols = ["競馬場", "競走年月日", "レース番号", "番号1", "オッズ"]
                        df_win_odds = df_wo_sub[df_wo_cols].rename(columns={"番号1": "馬番", "オッズ": "リアルタイム単勝オッズ"})
                        ...
                        for (track, date, r), df_r in grouped:
                            if len(df_r) >= 3:
                                ...
                                if win_rate >= target_win_rate:
                                    ...
                                    c_track = (df_master_payback["競馬場"] == track)
                                    c_date = (df_master_payback["競走年月日"] == date)
                                    c_race = (df_master_payback["レース番号"] == int(float(r)))
                                    df_pb = df_master_payback[c_track & c_date & c_race]
                                    ...
                                    if not df_pb.empty:
                                        ...
                                        for idx in [1, 2, 3]:
                                            ...
                                            if pd.notna(w_b1) and pd.notna(w_b2):
                                                ...
                                                if pb_pair == pair12:
                                                    payback_total += (amt12 / 100) * w_amt
                                                    hit_count += 1
                                                if pb_pair == pair13:
                                                    payback_total += (amt13 / 100) * w_amt
                                                    hit_count += 1
                                                if pb_pair == pair23:
                                                    payback_total += (amt23 / 100) * w_amt
                                                    hit_count += 1
    ...
    st.error(f"🚨 起動システムエラーを検知しました: {global_e}")
    st.write("GitHubのコピペ時に文字が化けたか、サーバーの初期化に失敗した可能性があります。")
