...
st.sidebar.markdown("---")
st.sidebar.sidebar.markdown("### 🎯 大将軍専用・大穴厳選フィルター")
...
if mode == "地方競馬（CSV一括丸投げ）":
    ...
    with c2: st.markdown(f"{'🟢 レース一覧' if df_race_raw is not None else '⚪ レース一覧未読'}")
    ...
    for track in tracks:
        ...
        for r in r_list:
            ...
            if len(df_r) >= 3:
                ...
                # 🕒 【アプデ】レース一覧データから「発走時刻」を安全に引っ張る！
                race_time_str = "時刻不明"
                if df_race_raw is not None:
                    try:
                        ...
                        if not df_time_search.empty:
                            ...
                            elif "発走予定時刻" in df_time_search.columns:
