...
# 🧠 【超次元パッチ】再実行されても大将軍がドロップしたデータを絶対に忘れない記憶装置！
    ...
    for track in tracks:
        ...
        for r in r_list:
            ...
            if len(df_r) >= 3:
                ...
                wide_combos = f"① {int(float(n1))}-{int(float(n2))}{o12}\n② {int(float(n1))}-{int(float(n3))}{o13}\n③ {int(float(n2))}-{int(float(n3))}{o23}"
